...
router.put('/teams/config', async (req, res) => {
    ...
    try {
       ...
       SET ${fields.join(', ')}
       WHERE id = $${paramIdx}
       RETURNING id, name, fallback_config, pii_redaction_enabled`, values);
        if (result.rows.length === 0) {
...
router.get('/logs', async (req, res) => {
    ...
    try {
        ...
        const [dataResult, countResult] = await Promise.all([
            db_1.pool.query(`SELECT * FROM request_logs
         WHERE ${whereClause}
         ORDER BY time DESC
         LIMIT $${paramIdx} OFFSET $${paramIdx + 1}`, [...values, limit, offset]),
            db_1.pool.query(`SELECT COUNT(*)::int AS total FROM request_logs WHERE ${whereClause}`, values),
        ]);
        return res.json({
...
router.get('/analytics/overview', async (req, res) => {
    ...
    try {
        ...
        const [metricsResult, budgetResult] = await Promise.all([
         ...
         FROM request_logs
         WHERE team_id = $1`, [teamId, todayMidnight.toISOString()]),
            db_1.pool.query(`SELECT monthly_limit_usd, current_spend_usd
         FROM team_budgets
...
router.get('/analytics/timeseries', async (req, res) => {
    ...
    try {
        const result = await db_1.pool.query(`SELECT time_bucket($1::interval, time) AS bucket,
              ${aggregation}::numeric AS value
       FROM request_logs
       WHERE team_id = $2 AND time >= $3 AND time <= $4
       GROUP BY bucket
       ORDER BY bucket`, [bucketInterval, teamId, from, to]);
        return res.json({
...
router.get('/teams/settings', async (req, res) => {
    const teamId = req.team_id;
    if (!teamId) {
        return res.status(400).json({ error: 'Team ID is missing' });
    }
    try {
        const result = await db_1.pool.query(`SELECT t.id, t.name, t.pii_redaction_enabled, t.fallback_config,
