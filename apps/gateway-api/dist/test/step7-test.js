...
function createMockOpenAIServer(port) {
...
function createMockAnthropicServer(port) {
    const server = http_1.default.createServer((req, res) => {
        let bodyText = '';
        req.on('data', (chunk) => {
            bodyText += chunk;
...
async function runTests() {
    ...
    // Clean DB and Redis
    try {
        await db_1.pool.query('DELETE FROM request_logs WHERE team_id = $1', [TEST_TEAM_ID]);
        ...
        await redis_1.redis.del(`team_spend:${TEST_TEAM_ID}`, `team_budget_limit:${TEST_TEAM_ID}`, `team_config:${TEST_TEAM_ID}`);
        // Clear BullMQ queues
    ...
    const workerProcess = (0, child_process_1.exec)('npx ts-node src/index.ts', {
        cwd: path_1.default.resolve(__dirname, '../../../worker'),
        env: {
            ...process.env,
    ...
    try {
        ...
        const sendProxyRequest = async (model) => {
        ...
        const dbRes1 = await db_1.pool.query('SELECT * FROM request_logs WHERE team_id = $1 ORDER BY time DESC LIMIT 1', [TEST_TEAM_ID]);
        if (dbRes1.rows.length === 0)
            throw new Error('No logs recorded for Test Case 1');
        ...
        await db_1.pool.query('UPDATE team_budgets SET current_spend_usd = 2.00 WHERE team_id = $1', [TEST_TEAM_ID]);
        await redis_1.redis.del(`team_spend:${TEST_TEAM_ID}`); // Evict cache
        ...
        await sleep(4000); // Wait for worker
        const dbRes2 = await db_1.pool.query('SELECT * FROM request_logs WHERE team_id = $1 ORDER BY time DESC LIMIT 1', [TEST_TEAM_ID]);
        ...
        await db_1.pool.query('UPDATE team_budgets SET current_spend_usd = 6.00 WHERE team_id = $1', [TEST_TEAM_ID]);
        await redis_1.redis.del(`team_spend:${TEST_TEAM_ID}`); // Evict cache
        ...
        await sleep(4000); // Wait for worker
        const dbRes3 = await db_1.pool.query('SELECT * FROM request_logs WHERE team_id = $1 ORDER BY time DESC LIMIT 1', [TEST_TEAM_ID]);
        ...
        await db_1.pool.query('UPDATE team_budgets SET current_spend_usd = 0.00 WHERE team_id = $1', [TEST_TEAM_ID]);
        await redis_1.redis.del(`team_spend:${TEST_TEAM_ID}`); // Evict cache
        ...
        await sleep(4000); // Wait for worker
        const dbRes4 = await db_1.pool.query('SELECT * FROM request_logs WHERE team_id = $1 ORDER BY time DESC LIMIT 1', [TEST_TEAM_ID]);
...
runTests();
//# sourceMappingURL=step7-test.js.map