...
async function runTests() {
    ...
    try {
        ...
        await redis_1.redis.del('bull:request-logs-failed:active', 'bull:request-logs-failed:wait', 'bull:request-logs-failed:id');
    ...
    try {
        ...
        const dbLogsRes = await db_1.pool.query('SELECT * FROM request_logs WHERE team_id = $1', [TEST_TEAM_ID]);
        ...
        const redisSpend = await redis_1.redis.get(`team_spend:${TEST_TEAM_ID}`);
        ...
        const failedJobIds = await redis_1.redis.lrange('bull:request-logs-failed:wait', 0, -1);
        ...
        const failedJobData = await redis_1.redis.hget(`bull:request-logs-failed:${failedJobIds[0]}`, 'data');
        if (!failedJobData)
            throw new Error('Failed job data not found in Redis');
...
runTests();
//# sourceMappingURL=step5-test.js.map