...
async function runTests() {
    ...
    try {
        ...
        const queueJobs = await redis_1.redis.keys('bull:request-logs:*');
        ...
        await redis_1.redis.del('bull:request-logs:wait', 'bull:request-logs:id');
        ...
        if (!reader2)
            throw new Error('No body in Anthropic response');
        let anthropicOutput = '';
        ...
        const jobIdsAnthropic = await redis_1.redis.lrange('bull:request-logs:wait', 0, -1);
        ...
        const jobDataAnthropic = await redis_1.redis.hget(`bull:request-logs:${jobIdsAnthropic[0]}`, 'data');
        if (!jobDataAnthropic)
            throw new Error('Anthropic Job data not found');
...
runTests();
//# sourceMappingURL=step4-test.js.map