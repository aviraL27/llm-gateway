...
async function runTests() {
  ...
  try {
    await pool.query('DELETE FROM api_keys WHERE team_id = $1', [payload.app_metadata.team_id]);
    await pool.query('DELETE FROM team_budgets WHERE team_id = $1', [payload.app_metadata.team_id]);
    await pool.query('DELETE FROM teams WHERE id = $1', [payload.app_metadata.team_id]);
  ...
  try {
    ...
    const firstKeyData = (await postResponse1.json()) as any;
    ...
    const secondKeyData = (await postResponse2.json()) as any;
    ...
    const keysList = (await getResponse.json()) as any[];
    ...
    const protectedData = (await validAuthResponse.json()) as any;
    ...
    const hash = hashApiKey(firstKeyRaw);
    const redisVal = await redis.get(`apikey:${hash}`);
    ...
    const redisValPostDelete = await redis.get(`apikey:${hash}`);
