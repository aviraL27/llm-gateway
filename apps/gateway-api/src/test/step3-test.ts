...
async function runTests() {
  ...
  try {
    ...
    await redis.del(`ratelimit:${apiKeyId}:${currentMinuteTimestamp}`);
    ...
    const cachedSpend = await redis.get(`team_spend:${TEST_TEAM_ID}`);
    const cachedLimit = await redis.get(`team_budget_limit:${TEST_TEAM_ID}`);
    ...
    const postUpdateSpend = await redis.get(`team_spend:${TEST_TEAM_ID}`);
    const postUpdateLimit = await redis.get(`team_budget_limit:${TEST_TEAM_ID}`);
    ...
    await pool.query('UPDATE team_budgets SET current_spend_usd = 10.00 WHERE team_id = $1', [TEST_TEAM_ID]);
    // Populate Redis
    await redis.set(`team_spend:${TEST_TEAM_ID}`, '10.00');
    ...
    // Check Redis
    const postResetSpend = await redis.get(`team_spend:${TEST_TEAM_ID}`);
