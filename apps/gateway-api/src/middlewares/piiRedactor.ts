...
function isValidLuhn(ccNum: string): boolean {
  ...
  if (!/^\d{13,16}$/.test(sanitized)) return false;
...
export async function piiRedactor(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT pii_redaction_enabled FROM teams WHERE id = $1',
      [teamId]
    );
    ...
    const ssnMap = new Map<string, string>();
    ...
    for (const msg of messages) {
      ...
      if (typeof content === 'string') {
        ...
        // Redact Emails
        content = content.replace(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g, (match: string) => {
          wasPiiDetected = true;
        ...
        // Redact SSNs
        content = content.replace(/\b\d{3}-\d{2}-\d{4}\b/g, (match: string) => {
          wasPiiDetected = true;
          if (!ssnMap.has(match)) {
        ...
        content = content.replace(/\b(?:\d[ -]*?){13,16}\b/g, (match: string) => {
        ...
        content = content.replace(/\b(?:\d{1,3}\.){3}\d{1,3}\b/g, (match: string) => {
        ...
        content = content.replace(/\+?\b[0-9]{1,4}[- \s]?(?:\([0-9]{1,3}\)|[0-9]{1,3})[- \s]?[0-9]{1,4}[- \s]?[0-9]{1,4}[- \s]?[0-9]{1,9}\b/g, (match: string) => {
        ...
        if (piiServiceUrl && content.trim().length > 0) {
          ...
          try {
            ...
            if (sidecarResponse.ok) {
              const resData = (await sidecarResponse.json()) as any;
