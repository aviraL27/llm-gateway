...
const failedQueue = new bullmq_1.Queue('request-logs-failed', { connection });
console.log('Worker starting...');
// BullMQ Worker to process request-logs
const worker = new bullmq_1.Worker('request-logs', async (job) => {
    const { team_id, api_key_id, provider, model, model_used, was_fallback = false, prompt_tokens, completion_tokens, cost_usd, latency_ms, status, pii_detected = false, } = job.data;
    ...
    try {
        ...
        await client.query(`INSERT INTO request_logs (time, team_id, api_key_id, provider, model, prompt_tokens, completion_tokens, cost_usd, latency_ms, status, pii_detected, model_used, was_fallback)
        ...
        await client.query(`UPDATE team_budgets
         ...
         WHERE team_id = $2`, [cost_usd, team_id]);
        // 3. UPDATE api_keys: last_used_at = NOW() (only if > 5 min since last update)
        await client.query(`UPDATE api_keys
         SET last_used_at = NOW()
         WHERE id = $1 AND (last_used_at IS NULL OR last_used_at < NOW() - INTERVAL '5 minutes')`, [api_key_id]);
